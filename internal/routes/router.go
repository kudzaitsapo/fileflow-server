...
func CreateRoutes() []Route {
	...
	routes = append(routes, Route{
		...
		Route{
			Pattern:      "PUT /v1/projects",
			Handler:      http.HandlerFunc(handlers.HandleProjectUpdate),
			RequiresAuth: true,
		},
		Route{
			Pattern:      "POST /v1/projects/re-generate-key",
			Handler:      http.HandlerFunc(handlers.HandleApiKeyRegeneration),
			RequiresAuth: true,
		},
		Route{
		...
		Route{
			Pattern:      "GET /v1/file-types",
			Handler:      http.HandlerFunc(handlers.HandleGetAllFileTypes),
			RequiresAuth: true,
		},
		Route{
